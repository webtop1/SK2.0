...
public class FrontService extends BaseService {
	...
	protected void home(BaseController controller) {
		// 首页图片 13
		Page<TbArticle> topPics = new FrontCacheService().getArticle(new Paginator(1, 4), 93);
		controller.setAttr("topPics", topPics);

		// 活动
		Page<TbArticle> newArticle = new FrontCacheService().getArticle(new Paginator(1, 3),92);
		controller.setAttr("newArticles", newArticle);

		// 新闻
		Page<TbArticle> news = new FrontCacheService().getArticle(new Paginator(1, 10), 99);
		controller.setAttr("news", news);

		// 我们的产品
		Page<TbArticle> products = new FrontCacheService().getArticle(new Paginator(1, 3), 98);
		controller.setAttr("products", products);

		// 旅游 4
		Page<TbArticle> travels = new FrontCacheService().getArticle(new Paginator(1, 9), 93);
